...
public class CartActivity extends BaseActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /*BBWebSocketClient.start(new MySessionHandler());
        stompSession.send("/app/order", "");*/
        connectStomp();
    }
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            for(int i=0;i<response.length();i++){
                                ...
                                Gson gson=new Gson();
                                List<Product> products=gson.fromJson(response.get("products").toString(),new TypeToken<List<Product>>(){}.getType());
